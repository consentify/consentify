...
- **Internal module split**: `packages/core/src/index.ts` has been split into focused modules under `packages/core/src/internal/` (`types`, `util`, `cookie`, `crypto`, `visitor`, `cloud`, `gcm`, `debug`). The public entry point and exports are unchanged; tree-shaking is preserved via `sideEffects: false`. The ESM bundle stays at 4.83 kB / 5 kB gzipped. The IIFE size budget was tightened from the interim 5.5 kB back down to 5.2 kB (current size 5.06 kB gzipped) — small headroom over the pre-split baseline to cover the re-export plumbing.
